...
- `/js/settings.js`: shared settings-page behavior and general preference runtimes, including themes, glow, mobile-view cookie syncing, notification-audio and hourly-beep preferences, account-backed Toast DM preferences, accessibility, title animation, oneko, guest inbox identity setup, and tooltips. Site tooltips use one shared 50-character wrapping measure and shrink to the longest rendered line. Desktop tooltips follow the pointer; mobile tap tooltips are limited to abbreviations generated from Markdown definitions and open beside their trigger. Capture-phase dismissal removes the active tooltip as soon as any subsequent element is tapped, before that element's own action runs
- `/js/fruity-dance.js`: the isolated Fruity Dance preference/runtime module, including its immediate settings persistence, custom asset controls, animated spritesheet and loop pickers, draggable sprite/reflection behavior, and related diagnostics. It receives debug-mode changes from the general settings runtime through `fridge:accessibility-change`; `main.js` calls its narrow `fridgeInitFruityDanceSettings()` hook after SPA content replacement so newly inserted settings controls reflect the active local preference
- `/js/sidebar-player.js`: shared sidebar, player, mobile-menu and notification UI. Notification revision checks run on initial load and when the tab becomes visible or focused; they do not use a permanent background polling interval.
...
- `/js/chat-conversation.js`: shared `/chat` and Toast website-chat conversation interactions, including polling, presence, typing, replies, reactions, hide/delete gestures, emoji, upload state, alerts, media controls, and server-controlled composer locking
- `/js/sidebar-player.js` checks a lightweight notification revision every 10 seconds in foreground and background tabs and fetches the inbox only when it changes. It maintains the full-width sidebar count shortcut, prefixes the page title with the unread count across direct and SPA navigation, and adds a fixed teal unread-count circle to the mobile menu control. Genuinely new events are limited to unseen entries at the leading edge of the ordered inbox, preventing older events entering the first page from being announced as new. On the desktop template, new inbox events play `/resources/audio/notification.mp3` at 30% volume when the device-local sound preference is enabled and audio has been unlocked by a pointer or keyboard interaction; blocked playback is discarded instead of playing later, and notification audio is disabled entirely on the mobile template. It also queues top-centre notification toasts that SPA-navigate to `/notifications` and marks notifications read when their target feed-post page is visited. A toast's dismissal timer and CSS animation only advance while the document is visible, preserving its full on-screen lifetime when it arrives in a background tab. The first successful mobile music or Toast Radio playback shows a persistent device-local guidance toast; swiping dismisses it permanently, while tapping dismisses it, opens the menu, scrolls to the player, and briefly highlights it
...
- `/js/bookmarks.js`: bookmark/save icons, anonymous bookmark storage, image modal behavior, and `/bookmarks` hydration. Feed, journal, and saved-post bookmark controls and journal edit controls share the `...` action menu's compact icon dimensions and hover colours; each theme applies the same surface, border, corners, and shadow to these controls on desktop and mobile. Journal cards and full articles use the shared `.journal-post-edit` class, exempting their edit icons from the older unframed edit-button rules
...
Base16 is a packaged flat interpretation of the Blackprint layout using the complete Base16 Default Dark `base00`–`base0F` palette. A theme-wide invariant removes inherited gradients, decorative background images, text shadows, filters, backdrop filters, and paper grain from the shell and components; functional theme thumbnails and Fruity Dance sprites remain visible. Solid Base16 surfaces cover cards, navigation, editors, forms, dialogs, chat, the player, and the explicit two-column mobile layout. Posts, grid cards, footer controls, and buttons share a dark offset shadow, while the sidebar player and footer share a neutral outline. Markdown formatting buttons use `base00`, visibly darker than their `base02` toolbar, with a bottom offset shadow; search buttons, the formatting guide, and the preview toggle remain shadowless. Native checked controls are restored and accented with `base0D`, keeping their indicators visible without custom image textures. The sidebar title and every title-animation preview use the selected `base0D` accent as a solid colour.

Every theme keeps the server-time label unboxed and gives BBCode/Markdown formatting buttons a visibly darker surface than the toolbar behind them. Base16 formatting controls retain a bottom offset shadow, and the serif-font and raw/formatted controls on Markdown reference pages share identical icon-button styling.
...
Gruvbox and Nord are packaged Blackprint derivatives built from the modular `/themes/template` scaffold. Gruvbox uses warm dark neutrals, cream text, bright interaction states, a smooth vertical tonal title gradient derived from the current accent, square corners, and offset shadows. Nord uses Polar Night surfaces, Snow Storm text, Frost interaction colours and title ramp, Aurora status colours, rounded panels, and softer depth. Both packages retain separate token, shell, component, content, player, mobile, and accessibility modules so their palettes and geometry can be adjusted without flattening component coverage into one stylesheet.

The colour scheme UI is an Appearance subsection directly beneath the theme picker. Classic shows its five colour inputs there; Base16, Gruvbox, and Nord each show one custom palette menu with named colour swatches and no separate arrow. Base16 offers red, orange, yellow, green, aqua, blue (default), purple, and brown; Gruvbox offers the same choices except brown, defaulting to green; Nord also omits brown and defaults to blue. Choices are restricted to each theme's palette rather than accepting arbitrary colours. Accent changes fade the viewport out and back as the new colour is applied, persist separately per theme, and sync to the account for logged-in users. The full-width reset control matches the main settings save action and restores only that theme's default. Choosing a different theme first resets that new theme's colour scheme, fades the viewport to black, reloads its shell immediately, and fades the new theme in; Reduce Motion shortens all of these fades to effectively immediate transitions. The title-animation picker's selected preview and all menu previews inherit the current theme's live title colours in every packaged theme.

Base16 replaces its `base0D` accent throughout the existing left button rules, hover states, links, focus, selection, and matching player/ASCII colours; its red and brown presets use black text on accent backgrounds to maintain contrast. Gruvbox replaces its primary green token throughout interactions, selection, matching title stops, and ASCII readouts while retaining its warm neutral surfaces; its red preset uses the darker background ink for readable text on accent surfaces. Nord's non-default options retint its surfaces, borders, text, title, shadows, and interaction colours to the selected hue while retaining each original token's OKLCH lightness and chroma. The swatches use the [Nord palette](https://www.nordtheme.com/docs/colors-and-palettes/); the colour model follows [CSS Color 4](https://www.w3.org/TR/css-color-4/#ok-lab). Status colours retain their meanings, and images and videos keep their original colours. Explicit high contrast takes precedence over accent presets. Run `node scripts/generate-theme-accents.mjs` to regenerate the preset stylesheets from the shared palette catalog and Nord tokens, or add `--check` to verify they are current.
...
External HTTP links use the on-site leaving-fridge.dev confirmation only while browsing `/feed` or `/mdpaste`. Other routes follow the link's normal same-tab or new-tab behavior.

Visible dates and times retain each component's existing text and styling. Hovering one uses the shared site tooltip to show its exact local timestamp as `DDth Month YYYY | HH:MM:SS`; journal-post dates are deliberately excluded.
...
Static site assets are grouped under `/resources`: general imagery in `/resources/images`, favicons and app icons in `/resources/icons`, bundled fonts in `/resources/fonts`, interface audio in `/resources/audio`, and the manifest plus ASCII clock data in `/resources/etc`.

The installable web-app manifest at `/resources/etc/site.webmanifest` uses `https://fridge.dev/` as its app id, launch URL, and navigation scope. Its icon URLs point to `/resources/icons`.
...
The development scaffolds are `/formatting/example` (`content.html`) and `/formatting/example_md` (`content.md`). The Markdown scaffold uses the site's explicit trusted Markdown renderer and viewer presentation inside the normal page content area, so repository-authored pages can include arbitrary raw HTML. Keep the restricted renderer for user-supplied mdpaste content; feed posts and comments use their own restricted renderer. Both scaffolds retain the standard session, theme, mobile template, and account greeting behavior and are excluded from deployment.

The Markdown viewer's `toggle serif font` control uses the same compact dimensions, theme surface, border, corners, and shadow as journal edit controls. It retains its native button behavior and `aria-pressed` state, with the foreground icon colour indicating hover, keyboard focus, or the enabled alternate font. Generic form-button and larger Markdown toolbar styles exclude this control so its appearance stays consistent on desktop and mobile.
...
- `theme_pref` cookie
- `themePref` localStorage entry; theme selection is device/browser-only and account `theme` fields are ignored
- `is_admin` cookie
...
The authenticated Toast account receives an orange bot-mode banner, restricted sidebar navigation, hidden notifications/bookmark controls, and a persistent Toast radio selection. `lib/bot-mode.php` adds the shared mode metadata and assets to desktop and mobile shells; `js/bot-mode.js` keeps disabled navigation and restriction notices working after SPA transitions. See [Toast](Toast#bot-mode) for the route policy and management permissions.

Blackprint’s settings save button explicitly keeps white text in its normal, hover, and active states; its dedicated important rules must stay consistent with the shared button colours.

## PreMiD activity

The root `presence.ts` is the standalone PreMiD activity source. It reports fridge.dev routes on desktop and mobile, re-reading the URL and DOM on every `UpdateData` event so SPA navigation is supported. Folder/index URLs, dynamic feed/journal posts, private conversations, shared pastes, and query-based views are supported. New routes without a dedicated status fall back to their rendered heading and then a generic fridge.dev status; add explicit statuses when introducing a feature. Private conversation IDs, form values, transfer tokens, and moderation search parameters are not included.

Music reports listening and artwork only while the shared mini-player audio is playing; paused playback reports music-library browsing. Journal and paste titles come from their current reading views, and internally rendered error pages take precedence over the requested route. The activity uses the site's current PNG favicon and strips both fridge.dev and legacy fridg3.org branding from document titles.

The root `metadata.json` configures matching for `fridge.dev`, `m.fridge.dev`, and their legacy `fridg3.org` equivalents. Load the metadata together with the compiled activity in PreMiD. The metadata retains its existing CDN artwork URLs; those directory names do not need to match the service name.

For an activity that disappears, enable lifecycle diagnostics in the website console with `localStorage.setItem('fridgePremidDebug', '1')`, then reload. Filter console output by `fridge.dev PreMiD`. Each extension tick logs receipt and completion of `setActivity`; a watchdog warns after 30 seconds without a tick, and update exceptions are logged. Completion means the API call returned, not that Discord displayed the activity. These logs contain no page contents or query parameters. Disable diagnostics with `localStorage.removeItem('fridgePremidDebug')` and reload. The watchdog does not send extra updates or override extension activity selection.
