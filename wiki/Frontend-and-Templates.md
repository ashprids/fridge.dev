...
- the shared PHP shutdown runtime maintains a permission-restricted JSON array of the latest 10,000 page accesses in `data/etc/access.json`. it records only non-API requests whose executing script is `index.php`, storing UTC time, connection IP, path without its query string, HTTP status, session username when available, and the request-time guest/user/admin role; request methods are not stored. the logged-in sidebar's `/chat?action=active-account-chat` discovery probe is explicitly excluded, so `/chat` appears only for genuine chat navigation. `/api/debug-access-logs` is admin-only, does not log its own polling requests, derives roles for legacy records from the current accounts data, and evaluates each unique logged IP against the current manual and source hard-ban lists. the access tab polls every five seconds, starts at the bottom, and provides `guests`, `users`, `admins`, and additive `hard-banned` filters. it displays `[time] [IP] [@username] [HTTP code] page`, omitting the username group for guests. only the IP text—not its brackets—turns red when hard-banned; other IP text is light grey, only the text inside username brackets is yellow, and status codes use green for 2xx, yellow-orange for 3xx, and red for 4xx/5xx. the development-data archive explicitly excludes this sensitive file
