...
`lib/theme-accents.php` validates the active theme's `theme_accent_{theme}` cookie against `themes/palettes/accents.json` and adds a `data-theme-accent` attribute to `<html>` before rendering. Invalid or missing cookies use the theme default. The same palette catalog is embedded as JSON for `/js/settings.js`, so the selector, server validation, and first-render CSS share one allowlist. JavaScript restores browser preferences and updates the attribute for immediate previews; other themes remove it, and their saved palette choices remain independent. Theme changes set the new theme's default colours and reload immediately. A short session marker lets the renderer add the incoming side of the black fade before styles paint, avoiding a flash of the new shell before it fades in. The renderer also refreshes shared runtime script versions for the built-in default as well as packaged themes.

`/formatting/example` is the HTML wrapper scaffold. `/formatting/example_md` follows the same shell, theme, session, and greeting flow, but reads its route-local `content.md` and renders it with `mdp_render_trusted_markdown()` from `tools/mdpaste/lib.php`. That explicit trusted renderer preserves arbitrary raw HTML, including nested `<div>` structures, and is reserved for repository-authored Markdown pages and journal posts. Its HTML-block detection still distinguishes Markdown URL and email autolinks such as `<example.com>` and `<person@example.com>`, along with inline elements such as `<abbr>`, when they begin a line. Abbreviations created by Markdown definitions and explicit `<abbr title="...">` elements both render through the site's `data-tooltip` behavior; definition text and existing tooltip attributes are protected from recursive abbreviation expansion. Definition-generated abbreviations receive a dedicated marker used to limit tap-triggered tooltips to those elements on mobile. User-content features must call the restricted `mdp_render_markdown()` entry point; mdpaste does so, while feed posts and comments retain their separate restricted renderer. The scaffold uses the shared Markdown viewer styles and enhancements inside the normal page content area, retaining the sidebar and selected desktop/mobile layout. Neither scaffold is uploaded by deployment; both are excluded in `.rsyncignore`.
