...
`sitemap.xml` is not deployed from git. The **regenerate sitemap** action writes a disk snapshot; the Nginx `/sitemap.xml` location serves fresh public URLs through `sitemap/index.php`, with a five-minute HTTP cache. Both use `fridge_sitemap_xml()` in `lib/sitemap.php`.

- Only reviewed public routes and published feed/journal/wiki content are included
- Every URL uses `https://fridge.dev`, matching the shared canonical tags
- Only content source timestamps are used for `lastmod`
- The disk snapshot must remain writable by PHP

The deployed Nginx config also redirects HTTP, `www.fridge.dev`, and `m.fridge.dev` to the HTTPS main domain. Mobile layouts are served on that main domain. See [Search and SEO](Search-and-SEO) for metadata policy, migration details, and post-deployment Search Console checks.
