...
- The settings page has a `moderator settings` section visible to admins and accounts with `isModerator`. Moderators can edit or delete non-admin feed posts and replies, and can use the existing IP-management, purge, and ban actions for non-admin content; admin-authored content remains outside moderator authority. Its `manage users` button opens `/settings/guests`, where either role can review registered accounts plus guest feed replies and IP-backed guestbook posts grouped by IP and ordered by newest activity, with ten IP groups per page. Account names, usernames, roles, and recorded IPs participate in search. Admin accounts are visibly protected for moderators, and every IP associated with an admin is also protected by server-side checks, including shared-IP cases. Permitted users can individually delete either guest content type, optionally record a reason when banning an IP, unban IPs across both posting surfaces, or purge all guest content from an IP after password confirmation
