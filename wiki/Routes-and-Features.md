...
- Guest replies that are mostly filter-list terms are rejected, and guest replies containing filtered text cannot be edited by guests after posting, though their delete action remains available. New filtered replies retain the unfiltered body as moderation-only metadata; moderators and admins can right-click a censored phrase to reveal its original text
- Reply edit/delete is allowed for the reply author, same-IP guest replies, admins, the original post owner, or accounts with `allowedPages` containing `comments`. Moderators may edit or delete replies by non-admin authors and use the matching IP moderation actions, but cannot act on admin-authored replies
...
- Feed post and reply management actions use a compact in-site dropdown rather than separate edit, delete, ban, and purge buttons; reply controls unrelated to management remain outside the menu. For posts with an associated IP, admins can jump directly to its filtered manage-guests view. Admins can ban an IP or purge guest replies by exact plaintext IP without deleting the feed post or changing the IP ban list. Guest replies from banned IPs are hidden from other viewers but remain visible to admins with an italic `(banned)` marker whose tooltip explains the admin-only visibility. IPs are not printed beside names; on desktop, admins can right-click every relevant guest or registered feed author name to reveal the exact IP recorded for that submission and double-right-click to copy it. On mobile, holding the non-selectable username opens a non-selectable in-site popup with `copy` and `ok` controls; the copy button briefly reads `copied` after activation. Content without historical IP metadata shows `No IP associated` instead of omitting the context display or substituting another address
...
- Owner/moderator/admin edit and delete flow. Guestbook messages use the shared guest filter; entries containing filtered text cannot be edited by their guest owner but can still be deleted, while moderators and admins can right-click each censored phrase to reveal its moderation-only original text
- Guestbook edit, delete, ban, and purge actions appear in one compact in-site dropdown when applicable, with an admin shortcut to the entry IP's filtered manage-guests view. Admins can ban an entry IP or password-confirm a purge of both feed replies and guestbook posts associated with that IP. Entry IPs are attached only for admins. On desktop they appear in an in-site tooltip when the guest name is right-clicked and are copied by a double-right-click; on mobile, holding the non-selectable guest name opens a non-selectable in-site popup with `copy` and `ok` controls, and the copy button briefly reads `copied`
...
- Successful saves briefly change the main button label to `saved!`; mobile-view changes then reload automatically using the newly persisted browser preference. Theme selection applies immediately through a black fade rather than waiting for the save button, resets the incoming theme's colour scheme to its default, and remains device-local
- Guest sessions can use the device-local notification-sounds setting on desktop and mobile. The mobile control remains hidden for account sessions.
- Includes accessibility toggles for mobile view, reduced motion, and debug mode, plus theme selection, a text glow toggle, optional cursor cat, and an in-site title-animation picker whose selected and menu previews inherit the active theme's title treatment. The colour-scheme reset spans the content width like the main save action, and palette accent changes use a fade before applying. On mobile, enabled debug mode adds a button beneath the menu control that toggles a full-screen debug console. Appearance → Misc also offers Fruity Dance; enabling it reveals a compact spritesheet picker generated from PNG files in `resources/images/fruity-dance`, a description-free live-preview picker for metadata-defined animation rows, and themed sliders for 25–200% speed and reflected-image height. A same-basename `.txt` file supplies each directory sheet's name, description, and row names. A final custom-spritesheet choice accepts a browser-local image and optionally a `.txt` file containing one animation name per row; all sheets use eight frames per row and fall back to the standard animation list when metadata is absent. While enabled with the no-upload custom placeholder selected, the loop title is blank and the loop and speed controls are disabled. Only in that state, `/music` exposes an isolated, deliberately misplaced final Cactile single whose title, caption, cover composites, clipping, colour faults, and transforms are regenerated at irregular random intervals; playing its `dance.mp3` source similarly corrupts the mini-player presentation without altering persisted metadata and hides downloads. Reflection remains usable for placeholder but is visually muted and retains the not-allowed cursor; at maximum corruption it visibly switches to `_custom2.png` at 40% or lower. Every applicable Fruity Dance control applies and persists immediately. FL Chan switches to the standard sheet's final held row, smoothly lifts, and tilts with horizontal pointer velocity while being dragged, then eases back down and upright on release. The reflection begins just below her feet, fades at its lower cut edge, and its slider changes the visible reflected depth; it renders outside the sprite's 110×128 pointer hitbox and cannot initiate dragging. Dragging smoothly compresses, softens, and offsets it slightly to suggest FL Chan being lifted. The normalized configuration persists to the account or local browser fallback. A Notifications section contains account-backed Toast Discord DMs for logged-in users (default on and disabled until Discord is linked), a device-local in-site notification sound toggle hidden on mobile, and the device-local hourly “beep beep!” toggle as its final item
- `/error/unknown` is an intentionally undeveloped browser-local progression route. Unauthorised visits show a pitch-black page with only a small centred padlock; after the terminal Fruity Dance journal sequence unlocks the route in localStorage, the black page instead gradually reveals an unselectable, extremely low-contrast `come back later` message
...
- Feed mention and reply alerts always populate the in-site inbox; linked accounts can independently enable or disable matching Toast DMs, whose delivery is documented on [Toast](Toast#notifications-and-website-integration)
...
- The complete admin-settings area is a collapsed-by-default disclosure, followed at normal section spacing by an admin-only separator outside the disclosure. Its `site management` group contains sitemap regeneration, notice management, and the admin-only `/settings/audit-log`; the system-information button appears only while debug mode is enabled. The audit log sorts successful moderator actions newest-first and provides case-insensitive search across usernames, IPs, actions, target details, and retained before/after content. Entries show readable action details and expandable native-style feed, reply, or guestbook cards for deleted content and edit comparisons, with at most 20 matching records per page
- The settings page has a `moderator settings` section visible to admins and accounts with `isModerator`. Moderators can edit or delete non-admin feed posts and replies, and can use the existing IP-management, purge, and ban actions for non-admin content; admin-authored content remains outside moderator authority. Its `manage users` button opens `/settings/guests`, where either role can review registered accounts plus guest feed replies and IP-backed guestbook posts grouped by IP and ordered by newest activity, with ten IP groups per page. Accounts are ordered as the reserved `admin` and `fridge` users, remaining admins, moderators, ordinary users, then banned users. Account cards link to a mixed chronological stream of native feed-post, reply, and IP-associated guestbook cards, paginated like `/feed` at 25 items per page; replies identify their parent author beside the username, and guestbook entries use the same compact relative timestamps as `/guestbook`. Ordinary account cards can be banned or unbanned by either role, while moderator/admin accounts are protected. An account ban restricts the account and its current and newly associated IPs until unbanned. Recorded-IP menus show the newest 100 addresses and link to the complete list. IPv6 labels show their last four groups with the full value in a site tooltip; each address has an icon-labelled right-click menu for lookup, shared-user review, password-confirmed purge, and admin-only hard banning. The same shortened labels and menus appear in `/account/admin/edit`. Account IP labels use the inline-code treatment without link underlines. Account names, usernames, roles, and recorded IPs participate in search. Admin accounts are visibly protected for moderators, and every IP associated with an admin is also protected by server-side checks, including shared-IP cases. Guest cards omit the status label unless banned; lookup remains available from the IP right-click menu, while purge, posting-ban, and admin-only hard-ban controls remain on the card. Individual guestbook delete controls are omitted from this combined moderation view.
- The moderator section's `manage bans` button opens `/settings/restricted-ips` for admins and moderators. It lists all content-upload bans with their optional reason (shown as `No reason provided` when blank), banning account, ban time, adjacent post-history and unban controls, and a combined live/deleted post-history view. Retained feed posts and replies use their original Markdown or legacy formatting and native feed cards; guestbook entries use guestbook post cards. Live cards omit a redundant `live` label and navigate to the exact post or reply when clicked, while deleted snapshots remain non-navigable and explicitly marked. While a ban remains active, its title and optional reason appear once per browser in a popup; the fuller `/notifications` entry retains the contact instructions and `mailto:ashton@fridge.dev` link
- Admins can open `/settings/banned-ips` to edit the separate hard-ban IP list; valid IPv4 and IPv6 addresses may be separated by spaces or newlines, and Nginx redirects matching clients away from all pages and static files to `/error/blacklisted`. Authenticated moderators and admins bypass hard bans and all other IP-derived posting limits while the same address remains restricted for guests and ordinary accounts; access-log moderation controls are documented on [Debug Mode](Debug-Mode#access-tab), while the enforcement model is documented on [Restrictions and Moderation](Restrictions-and-Moderation)
...
- Admin site management includes **restore backup** below **manage notices**. It previews automatic-backup ZIP counts locally, requests the account password and final red confirmation, then uploads and restores under locked maintenance. A shared progress popup follows admin navigation, resumes interrupted uploads from browser storage, and links home when complete. See [Backup Data](Backup-data#restore-from-settings).
...
- Provides a `create account` button above the account list
...
- Includes a `moderator` checkbox directly beneath `admin`; the same flag is editable from `/account/admin/edit`
...
- Administrators can revoke a claimed chat without deleting its history, clearing its recipient ownership so a different browser or account can claim the link
...
- The recipient's first full chat view shows an up-to-date security/help popup explaining browser/account ownership, encrypted storage and deletion, attachments and voice notes, desktop right-click controls, mobile reply/delete/hide gestures, reactions, and the difference between hidden and deleted messages
...
- Non-manager recipients with an active linked chat get a sidebar button above the mini-player/sidebar footer, including cookie-linked guests; it matches the notification shortcut, follows the footer's rendered width across themes, and counts conversations whose latest incoming message has not been viewed in that browser. Logged-in recipients can delete their chat themselves
...
- Chat polling is SPA-lifecycle-aware. If a conversation ends while its recipient is viewing another SPA page, stale chat timers stop and the active-chat shortcut is removed without navigating away. Manager-ended chats create a persisted `Conversation ended` inbox notification for linked non-admin recipients and refresh that inbox immediately; cookie-only guests receive a transient toast because they have no notifications-page identity. Confirming `end chat` yourself stores a short-lived session marker and creates no ended notification
...
- Plain HTTP(S) URLs in messages become safe clickable links and produce up to four end-of-message previews. YouTube, Vimeo, Dailymotion, Giphy, and direct-video URLs render as borderless media without a surrounding card body; direct images and audio use inline media; file and ordinary webpage URLs use compact cards. Image responses are recognized by HTTP content type even when their paths have no image extension, and ordinary cards include `og:image`/`twitter:image` artwork when available. Card headings use the page title for up to two lines and secondary text uses its meta description for up to three lines when available. Metadata is fetched once when the message is sent and stored inside the encrypted message; a versioned, bounded backfill updates older metadata as chats are opened. The fetcher supports cURL or PHP streams, rejects private/reserved IPs, does not follow redirects, accepts HTML or direct image responses, uses short timeouts, caps the response at 256 KiB, and has both DOM and lightweight meta-tag parsers. Missing metadata falls back to the hostname/path label
...
- Messages can visually reply to a previous message. On desktop, the reply/react/delete action menu opens only on right-click. On mobile it is suppressed: holding a message opens the emoji reaction picker, swiping left on your own message or right on the other person's message starts a reply, and swiping your own message right reveals a trash icon. Swiping the other person's message left toggles viewer-specific hiding; hidden content remains intact for the other participant and appears as a dimmed `message hidden` placeholder to the viewer. The in-site deletion warning is shown until the user confirms it once, then that browser deletes future messages immediately. Deleted messages stay in place as dimmed `message deleted` placeholders, but deleted messages and replies targeting them do not retain quoted-reply panels
- Chat buttons use flat fills without gradients or shadows across every theme. Owned message bubbles use a solid medium teal with white content, while quoted-reply panels use a softer charcoal background with white text. The desktop right-click action menu uses compact, lightweight controls with a small gap between actions
- On the mobile template, an open conversation uses an app-like full-viewport layout: the site page is locked in place, only the message history scrolls, the compact `fridge.dev chat` header and square, shadowless composer controls remain visible, controls respect the visual viewport and device safe areas as the keyboard opens or closes, and the navigation drawer continues to overlay and blur the conversation without placing a click-blocking layer above the drawer. Tapping blurred content closes the drawer without activating the underlying page, and opening it preserves the current scroll position. The mobile `+` control directly opens the device image picker, while a microphone button styled identically to Send toggles a bounded recorder popover with Record/Stop, timer, preview, Use, and Discard controls; closing the popover also stops an active recording. On desktop, the `+` choices open as a compact popover anchored above the composer rather than occupying a full grid row
...
- Each participant message creates a targeted notification for every administrator titled `New chat message`, links back to the conversation, and identifies the recipient label in its description. Each admin retains only their newest chat-message notification; a newer one replaces older chat-message records without affecting unrelated alerts. Notification toasts remain queued while an administrator is on `/chat` and resume after leaving chat routes
- Each manager message creates a `New chat message` targeted notification for the linked non-admin recipient account. Each recipient retains only their newest chat-message notification. Opening that conversation, directly or through SPA navigation, automatically marks notifications linked to that chat as read; cookie-only guests continue to use the active-chat unread indicator because they have no account inbox identity
- New incoming and locally sent messages automatically scroll the conversation history to the bottom
- Chat routes hide and do not update the shared page-view counter
...
- Replies are sent manually from `ashton@fridge.dev`
- Accepted submissions are stored under `data/contact/*.json` with both their moderation-visible plaintext IP and the existing hashed rate-limit identity
- Each accepted submission places its IP on a six-hour cooldown. The server rejects further submissions during that window, while the form greys out its disabled send button, replaces `send` with a live `HH:MM:SS` countdown, and explains the anti-spam cooldown in the button tooltip
...
- After a successful submission, the redirected form opens an in-site `form submitted` popup explaining that the reply will arrive on the feed or from `ashton@fridge.dev`
...
- Lists submissions newest-first with their associated IP; legacy submissions that predate plaintext IP storage show `No IP associated`
- Discord notification failures remain stored for diagnostics but are hidden from the dashboard while the website is in developer mode
...
- Preview and published links share the site-wide Markdown renderer documented by `/formatting/markdown` and its canonical `formatting/markdown/formatting.md` source; rendered list items use consistent vertical spacing for readability, top-level headings have extra space from the blocks above and below them, and every raw-source view highlights YAML front matter separately from the Markdown body rather than using automatic language detection or interpreting a closing front-matter delimiter as a Setext article heading
...
- The renderer preserves nested blockquotes, attributed quotes beginning with `> [!QUOTE Name]`, mixed and Roman-numbered child lists, nested tasks, internal and angle-bracket links, protected inline code, GitHub alerts, Font Awesome `!fa style icon-name` icons, the inline fridge.dev `!frdg` icon, mathematical notation, constrained tables, and feed-style audio/video players
...
New top-level feed post files use `v2` as their first line, followed by the existing `@username`, timestamp, and body fields. A `v2` body uses a deliberately limited Markdown renderer supporting bold, italic, underline, strikethrough, highlight, links and uploaded media, blockquotes, attributed quotes beginning with `> [!QUOTE Name]`, inline/fenced code, tables, spoiler text, Font Awesome icons, the inline fridge.dev `!frdg` icon, and nested ordered/unordered lists. A standalone HTTP(S) URL ending in a supported image, audio, or video extension is embedded automatically; query strings do not prevent extension detection. Rendered feed lists use the same item spacing in published posts and previews. Lists and attributed quotes are syntax-only features without dedicated toolbar buttons; other unsupported Markdown and arbitrary HTML render literally. The shared Markdown toolbar keeps media uploads, pasted-image handling, image cropping, and voice notes; voice recording sits in the scrolling formatting group, while fixed guide and preview buttons remain on the right. The guide opens `/formatting/markdown/feed` in a new tab. Files without the marker remain legacy BBCode posts with their existing renderer and styling so saving one cannot silently reinterpret its BBCode; their client-side renderer also embeds raw media URLs.

Across themes, BBCode and Markdown formatting buttons use a darker surface than their surrounding toolbar. Account-banned users and IP-banned guests receive the applicable restriction notice without reply or guestbook editor controls, and reply-target buttons are hidden while the viewer cannot post.
...
- `/others/toast-discord-bot/chat` is the public image-capable Toast conversation, keyed to an account or guest IP and limited to 100 visitor messages per London calendar day
- `/others/toast-discord-bot/chat/history` is the admin/Toast viewer for stored website conversations, with IP/username search, ten conversations per page, and permanent conversation/image deletion

### `/others/fridge-builds-websites`

Wrapper/marketing page for custom website work. Its repository-authored `content.md` uses Markdown throughout, including the service and pricing tables, linked example images, and enquiry link.

### `/others/fridge-builds-websites/submit`

- Public website-commission form styled with the shared contact/form controls
- Collects name, email, preferred contact method, one or more website categories, a project description, optional budget, preferred payment method, and explicit acceptance of the displayed commission terms
- Selecting `Other` for contact, website type, or payment reveals a required text field; selecting a budget reveals a required amount-and-currency field
- Selecting the affirmative terms option opens the shared site popup for a typed signature; the submit button remains disabled until the visitor signs, and both the agreement and signature are enforced server-side
- An admin-only `commissions open` checkbox controls the form. Closing commissions stores the setting under `data/etc`, removes the form for visitors, and makes the submission endpoint reject requests until an admin reopens it
- Uses the contact form's CSRF, honeypot, minimum-completion-time, arithmetic security question, posting restriction, banned-IP, and 48-hour per-IP cooldown protections
- Accepted requests are saved under `data/website-commissions` and sent through Toast to Discord channel `1547229814321188995`; notification failures are retained with the saved request

## Formatting / Examples / Errors

- `/formatting`
- `/formatting/example`
- `/formatting/example_md` (Markdown source rendered inside the standard site shell)
- `/error/403`
- `/error/404`
- `/error/50x`
- `/error/wip`
