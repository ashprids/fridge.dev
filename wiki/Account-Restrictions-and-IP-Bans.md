...
Source files are tokenized in fixed-size chunks and compiled into a binary range index beneath `data/etc/banlists/index/`. Exact IPs and CIDRs share the same fixed-width range representation, split by IP version and first address byte, so each authorization request reads only the relevant small bucket. The cache key includes every source path's inode, size, modification time, and change time; changing, adding, or removing a source list therefore builds and atomically publishes a new index. Concurrent builders are serialized with a file lock. Interrupted build directories are removed by the next locked index access, and superseded signature versions older than one hour are pruned. If the index cannot be created, matching falls back to the bounded-memory source scanner rather than failing the authorization request.
