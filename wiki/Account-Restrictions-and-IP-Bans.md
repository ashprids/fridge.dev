...
Admins can disable **strict hard bans** in the admin-only section of `/settings`. The setting defaults to enabled and is stored globally in `data/etc/hard-ban-settings.json`. When disabled, previously propagated associated IPs are removed from the manual hard-ban list while each identity group's original banned IP remains. Only the client's current IP is then checked against the manual and source hard-ban lists: a matching browser identifier does not deny access from another IP and does not append that IP to `hard-banned-ips.txt`. Recognized identifiers still update their observed IPs, last-seen timestamp, and user-agent hash for administrative continuity. The blacklist page then advises clients to disable VPNs, proxies, or other IP-masking tools.
