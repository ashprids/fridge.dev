...
try {
    foreach (fridg3_theme_accent_palettes() as $theme => $palette) {
        ...
        foreach ($palette['colors'] as $choice => $hex) {
            ...
            check_accent(fridg3_normalize_theme_accents([$theme => $choice]) === [$theme => $choice], "Rejected $theme/$choice");
            $_COOKIE['theme_accent_' . $theme] = $choice;
            $html = fridg3_inject_theme_accents($template, $theme);
            ...
            check_accent(fridg3_inject_theme_accents($html, $theme) === $html, "Duplicate injection for $theme");
        }
        foreach (['not-a-color', '#00FF00', '"><script>alert(1)</script>', ['red'], null] as $invalid) {
            check_accent(fridg3_normalize_theme_accents([$theme => $invalid]) === [], "Accepted invalid $theme accent");
    ...
    check_accent(fridg3_normalize_theme_accents(['classic' => 'red']) === [], 'Classic must keep its independent colour contract');
    check_accent(!str_contains(fridg3_inject_theme_accents($template, 'classic'), 'data-theme-accent='), 'Accent leaked into Classic');
    check_accent(!str_contains(fridg3_inject_theme_accents($html, 'default'), 'data-theme-accent='), 'Accent leaked when switching to default');
} finally {
