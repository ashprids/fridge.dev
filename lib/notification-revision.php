...
function fridge_notification_revision_path(): string {
    return dirname(__DIR__) . '/data/etc/notification-revision.txt';
}

function fridge_notification_revision_read(): string {
    $path = fridge_notification_revision_path();
    $value = is_file($path) ? trim((string)@file_get_contents($path)) : '';
    return $value !== '' ? $value : '0';
}

function fridge_notification_revision_touch(): bool {
    $path = fridge_notification_revision_path();
