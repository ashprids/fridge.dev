...
function toast_reply_context(array $post, array $replies, string $parentId): array {
    ...
    foreach ($replies as $reply) {
        if (!empty($reply['isGuest']) && !empty($reply['ip']) && fridg3_feed_is_ip_banned((string)$reply['ip'])) continue;
    ...
    $project = static fn(array $entry): array => [
        ...
        'author'=>(string)($entry['username']??''), 'text'=>fridg3_toast_feed_plain_text((string)($entry['body']??''), 8000),
    ];
    ...
    for ($i=0; $i<count($queue); $i++) {
        ...
        $entry['text']=fridg3_toast_feed_plain_text($entry['text'], min(2000, $remaining));
