...
if (!fridge_toast_is_current_user() || fridge_current_user_posting_restricted()) toast_reply_response(['ok'=>false,'error'=>'Only an unrestricted Toast account can generate replies.'],403);
...
$raw=@file_get_contents(fridge_feed_posts_dir().'/'.$postId.'.txt');
if ($raw===false) toast_reply_response(['ok'=>false,'error'=>'The feed post is no longer available.'],404);
$post=fridge_feed_parse_post($raw);$post['id']=$postId;
try { $context=toast_reply_context($post,fridge_feed_load_replies($postId),$parent); }
