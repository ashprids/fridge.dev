...
Prefer overriding variables over duplicating component rules. Add direct component overrides where the theme changes shape, spacing, typography, imagery, or layout. Classic exposes account-synced custom values for `bg`, `fg`, `border`, `subtle`, and `links`. Base16, Gruvbox, and Nord each expose one palette-limited accent selector; their shared allowlist and defaults live in `/themes/palettes/accents.json`, with validation and first-render cookies handled by `/lib/theme-accents.php`. All colour controls remain in the Appearance → Color scheme subsection directly below the theme picker. Palette choices use the shared custom menu rather than a visible native select, accent changes fade out and back, and changing themes resets the incoming scheme before the immediate fade-and-reload transition. Colour-scheme reset actions span the settings content width. Title rules must cover `.title-animation-preview` and its `.title-letter` children as well as the live desktop and mobile titles; a theme that removes gradients must provide a visible solid text fill for those previews. Regenerate preset CSS with `node scripts/generate-theme-accents.mjs` after changing palettes or Nord's base tokens, and verify it with `--check`. Nord presets preserve each tinted token's OKLCH lightness and chroma while taking their hue from the selected palette colour; status colours and media are not filtered. Presets defer to explicit high-contrast mode. Adding color controls for another theme requires coordinated changes in `/js/settings.js` and `/api/settings/index.php`.
